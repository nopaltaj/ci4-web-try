...
-moz-box-shadow: 0px 4px 11px 0px rgb(0 0 0 / 53%); ">
    ...
    <div class="row " style="width: auto;">
        ...
        <div class="m-5 col">
            ...
            <form action="<?= base_url('dataBarangEditProses') ?>" method="POST">
                <input type="hidden" class="form-control" id="id" name="id" value="<?= esc($data_barang['id']); ?>">
                <div class="input-group mb-3">
                    <select class="form-select" name="status" id="">
                        <option value="PENDING" <?= esc($data_barang['status']) == 'PENDING' ? 'selected' : '' ?>>Pending
                        </option>
                        <option value="SUCCESS" <?= esc($data_barang['status']) == 'SUCCESS' ? 'selected' : '' ?>>Success
                        </option>
                    </select>
